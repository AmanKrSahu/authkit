...
| 1.2  | **Add `return` in OIDC `loginInteraction`** after the MFA-required response                                                                                                     | SEC-C2          | P0       | S          | none            | Closes MFA bypass over SSO                           | Todo     |
| 1.3  | **Rotate ALL secrets** (JWT×4, OIDC RSA keypair, cookie keys, authenticator key, Google secret, Resend key, DB password); move to a secrets manager; URL-encode the DB password | SEC-C3, SEC-L11 | P0       | M          | secrets manager | Restores trust base integrity                        | Todo     |
| 1.4  | **Remove the `AUTHENTICATOR_APP_SECRET` default**; require + validate ≥32B entropy at boot                                                                                      | SEC-C4          | P0       | S          | 1.3             | Prevents predictable TOTP-encryption key             | Todo     |
| 1.5  | **CORS exact-origin allowlist** — remove `origin.includes(...)` substring checks                                                                                                | SEC-H1          | P0       | S          | none            | Closes credentialed cross-origin bypass              | Todo     |
...
| 1.7  | **Configure `trust proxy` + `req.ip`; add per-account login lockout**                                                                                                           | SEC-H3          | P0       | M          | none            | Restores brute-force protection                      | Todo     |
