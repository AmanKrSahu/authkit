...
| 2.12 | Low-tier cleanup: pin JWT `algorithms`, email-verify gate, MFA re-auth, refresh single-use, gate `/docs`, password max-length, remove dead `sessionToken`, server-side device fingerprinting | SEC-L1–L9 | P3       | M          | none         | **Done** |
...
| 2.15  | Move bcrypt out of the register transaction                                                    | perf QW-5        | P2       | S          | ~100ms less connection hold per register               | **Done** |
| 2.16  | Batch Redis session deletions (`deleteCacheMany`)                                              | perf QW-6        | P2       | S          | M round-trips → 1 on revoke-all                        | **Done** |
| 2.17  | Parallelize/queue registration emails                                                          | perf QW-4        | P2       | S          | ~1 email RTT off register latency                      | **Done** |
